...
public class Predicate {
    ...
    Predicate selectSubPred(Schema sch) {
        ...
        for (Term t : terms)
            if (t.appliesTo(sch))       // Term内のExpressionが定数なら使えるし、フィールド名ならそのフィールドがsch内に存在すれば使える
                result.terms.add(t);
    ...
    public Constant equatesWithConstant(String fldname) {
        for (Term t : terms) {
            Constant c = t.equatesWithConstant(fldname);    // 左右のExpressionがfldnameというフィールドと定数cのペア
            if (c != null)
                return c;
        }
        return null;
    ...
    public String toString() {
        ...
        while (iter.hasNext())
            result += " and " + iter.next().toString();     // lhs.toString() + "=" + rhs.toString(); 左右のExpression
        return result;
    }
